-- penaltiescontrol.lua — CSP online script (12h Curitiba)
--
...
local function makeZone(category, startPos, endPos, maxWheelsOut, rules, deadline, maxGas)
...
local config = {
  ...
  cutZones = {
    makeZone('SD1', cfg.cutZoneStart, cfg.cutZoneEnd, cfg.cutMaxWheelsOut,
      ...
      cfg.cutSlowdownDeadline, cfg.cutSlowdownMaxGas),
    makeZone('SD2', cfg.cutZone2Start, cfg.cutZone2End, cfg.cutZone2MaxWheelsOut,
      ...
      cfg.cutZone2SlowdownDeadline, cfg.cutZone2SlowdownMaxGas),
  },
...
ac.onCarJumped(0, function()
  if state.ui.clock <= state.tow.ownJumpUntil then
    ac.log('penaltiescontrol: car jumped (hold)')
    return
  end
  state.list.jumped = true
  if not state.list.prevInPit then state.tow.jumpPending = true end
  ac.log('penaltiescontrol: car jumped')
end)

ac.log('penaltiescontrol: section=' .. tostring(__cfgSection__)
  .. ' mode=' .. config.mode
  ...
  if senderCarIndex == -1 then
    ...
    elseif kind then
      if CODE80_KINDS[kind] then
        if not state.code80 then ac.log('penaltiescontrol: CODE-80 start (' .. kind .. ')') end
        state.code80 = kind
      elseif kind == 'GREEN FLAG' and state.code80 then
        state.code80 = nil
        state.code80Ended = true
        ac.log('penaltiescontrol: CODE-80 end')
      end
...
local LIST_KEY = 'penaltiescontrol.list'
local Rules = {}
...
local function applyHold(long)
  ...
  Rules.zero()
  ac.log(string.format('penaltiescontrol: hold %d s', seconds))
...
local function applyTowHold(tow, damage, reason)
  ...
  ac.log(string.format('penaltiescontrol: tow hold %d s (tow %d, repair %d; engine %.3f gearbox %.3f suspension %.3f'
...
local function applyExpiredDSQ(expired, viaPit)
  ...
  Rules.zero()
  ac.log(string.format('penaltiescontrol: DSQ for unpaid DT0 (%s), lap completed %s',
...
function Rules.sync(gs, where, lapsLost)
  ...
  l.written = gs.p
  ac.log(string.format('penaltiescontrol: PAC %s game=%d/%d before=%s after=%s', where, gs.t, gs.p, before,
    listDump()))
...
function Rules.line(viaPit, prev, g, lapCount)
  ...
  if viaPit then
    ...
    if state.code80 and l.items[1] then
      ac.log('penaltiescontrol: pit pass during CODE-80, nothing paid')
  ...
  ac.log(string.format('penaltiescontrol: line %s game before=%d/%d after=%d/%d table before=%s after=%s',
...
local function onSlowdownUnpaid(cat, endOfLap, inPit)
  ...
  ac.log(string.format('penaltiescontrol: slowdown %s unpaid, end of lap=%s', cat, tostring(endOfLap)))
end
...
local function onPitViolation(car, r)
  ...
  ac.log(string.format('penaltiescontrol: pit car.index=%d sessionID=%d DSQ', car.index, car.sessionID))
end
...
local function checkCutZone(zoneIndex, zone, lapCount)
  ...
  if not state.cutPassPenalized[zoneIndex] and car.wheelsOutside > zone.maxWheelsOut then
    ...
    ac.log(string.format('penaltiescontrol: cut %s spline=%.4f wheelsOut=%d penalty=%s',
...
-- Other pending penalties, shown next to the "+N" badge: "SD1 DT0  PAC DT1"
local function pendingText(items)
  local parts = {}
  for i = 2, #items do parts[#parts + 1] = items[i].cat .. ' ' .. itemPriority(items[i]) end
  return table.concat(parts, '  ')
end

-- Light gray; with a yellow or blue flag it blinks in the flag color (back to light gray when the blink is off);
-- VSC / SC / CODE-80: fixed yellow; green flag message: green
local function borderColor(flag)
  if CODE80_KINDS[flag] then return BORDER_YELLOW end
  if flag == 'GREEN FLAG' then return BORDER_GREEN end
...
-- "+N" badge, right-aligned at xRight; returns its left edge
local function drawBadge(text, xRight, y, s)
  local size = 13 * s
  ui.pushDWriteFont(FONT_TITLE)
  local tw = ui.measureDWriteText(text, size).x
  local p1 = vec2(px(xRight - tw - 12 * s), px(y))
  local p2 = vec2(px(xRight), px(y + 18 * s))
  ui.drawRectFilled(p1, p2, BORDER_YELLOW, px(9 * s))
  ui.dwriteDrawText(text, size, vec2(px(p1.x + 6 * s), px(p1.y + 1 * s)), rgbm(0.08, 0.08, 0.08, 1))
  ui.popDWriteFont()
  return p1.x
end
...
function script.drawUI()
  ...
  -- Fixed layout: penalty message box on top, slowdown box always right below it (same place even when the
  -- message box is empty), driver swap panel below. Fixed size; long texts wrap inside the message box.
  -- Legibility correction only (no proportional scaling): sizes grow with the square root of the resolution,
  -- from 1x at 1080p up to 1.5x, so on a 4K screen the boxes are bigger but take a smaller part of the screen.
  local s = math.min(math.max(math.sqrt(h / 1080), 1), 1.5)
  local boxW = math.floor(440 * s)
  local msgH, sdH = math.floor(64 * s), math.floor(50 * s)
  local gap = math.floor(6 * s)
  local x = math.floor(w * 0.5 - boxW * 0.5)
  local yMsg = math.floor(h * 0.18)
  local ySd = yMsg + msgH + gap

  -- Penalty message box: the drive-through being paid (position 0) stays fixed; a new penalty is shown for
  -- NOTICE_SECONDS; "+N" = other penalties pending. A hold shows its countdown. During CODE-80 the title carries
  -- VSC / SC / CODE-80 and the border is fixed yellow. Border light gray, blinking yellow / blue with the flag.
  if not state.dtDsqActive and not state.pitDsqActive then
    ...
    local title, text
    local flag = state.code80
    local rcTitle = state.code80 and (TEXTS.rcTitle .. '  ' .. state.code80) or TEXTS.rcTitle
    if notice then
      title, text = notice.title, notice.text
      flag = notice.flag or flag
    elseif state.hold then
      title = TEXTS.rcTitle .. '  ' .. TEXTS.holdTitle
      text = string.format(TEXTS.hold, mmss(state.hold.untilT - state.ui.clock), state.hold.text)
    elseif items[1] then
      title, text = rcTitle, itemText(items[1])
    elseif state.code80 then
      title, text = rcTitle, TEXTS.code80
    end
    if title then
      local p1 = vec2(x, yMsg)
      local p2 = vec2(x + boxW, yMsg + msgH)
      local border = borderColor(flag)
      drawPanel(p1, p2, border, s)
      drawText(title, FONT_TITLE, 16 * s, vec2(p1.x + 16 * s, p1.y + 5 * s), COLOR_TITLE)
      if #items > 1 then
        local left = drawBadge(string.format('+%d', #items - 1), p2.x - 14 * s, p1.y + 6 * s, s)
        drawTextRight(pendingText(items), FONT_MONO, 12 * s, left - 8 * s, p1.y + 8 * s, COLOR_TEXT)
      end
      drawSeparator(p1, p2, p1.y + 27 * s, s)
      ui.pushDWriteFont(FONT_TEXT)
      ui.setCursor(vec2(math.floor(p1.x + 16 * s), math.floor(p1.y + 32 * s)))
      ui.dwriteTextAligned(text, 14 * s, ui.Alignment.Start, ui.Alignment.Start,
        vec2(boxW - 32 * s, msgH - 34 * s), true, COLOR_TEXT)
      ui.popDWriteFont()
    end
  end
